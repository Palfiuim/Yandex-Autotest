...
def positive_assertion(name):
	...
	kit_response = sender_stand_request.post_new_client_kit(kit_body_positive, auth_token)
...
def negative_assertion(name):
	...
	response = sender_stand_request.post_new_user(data.user_body)
	auth_token = response.json().get("authToken")
	kit_response = sender_stand_request.post_new_client_kit(kit_body_negative, auth_token)
...
def test_create_kit_no_name_get_error_response(): 
    current_kit_body_negative = data.kit_body.copy()     #   Deleting “name” from a query 
    current_kit_body_negative.pop("name")    
    response = sender_stand_request.post_new_user(data.user_body)
    auth_token = response.json().get("authToken")
    kit_response = sender_stand_request.post_new_client_kit(current_kit_body_negative, auth_token)
    assert kit_response.status_code == 400
